...
document.querySelector('#tabla')?.addEventListener('click',(ev)=>{if(ev.target.closest('.cell-editor'))return;const td=ev.target.closest('td.editable');if(!td||!editMode)return;ev.stopPropagation();const ri=+td.dataset.ri,col=td.dataset.col,row=currentRows[ri],id=currentIdCol?row[currentIdCol]:null;if(!id){alert(`No se encontró ID (${ID_HEADER})`);return;}const kN=N(col),isDate=S_DATE.has(kN),isInt=S_INT.has(kN);if(td.querySelector('input,select'))return;const old=td.textContent||'';td.innerHTML='';let inp;let shouldSaveOnBlur=true;let isFinished=false;if(col==='COMENT.'){inp=document.createElement('select');inp.style.width='100%';COMMENT_OPTIONS.forEach(o=>{const opt=document.createElement('option');opt.value=o;opt.textContent=o||'—';if(old.trim()===o)opt.selected=true;inp.appendChild(opt);});}else{inp=document.createElement('input');inp.style.width='100%';inp.style.boxSizing='border-box';if(isDate){inp.type='date';const m=/^(\d{2})-(\w{3})-(\d{2})$/.exec(old.trim());if(m){const i=monES.indexOf(m[2].toLowerCase());if(i>=0)inp.value=`20${m[3]}-${String(i+1).padStart(2,'0')}-${m[1]}`;}}else if(isInt){inp.type='number';inp.step='1';inp.min='0';const n=parseInt(old,10);if(!isNaN(n))inp.value=String(n);}else{inp.type='text';inp.value=old;}}const w=document.createElement('div');w.className='cell-editor';w.appendChild(inp);td.appendChild(w);inp.focus();inp.select?.();async function save(){if(!idToken){alert('Accede primero');td.textContent=old;return;}let v=(inp.value||'').trim();if(v===old.trim()){td.textContent=old;return;}td.textContent='Guardando…';try{const res=await jsonp(`${B}?route=orders.update&idToken=${encodeURIComponent(idToken)}&id=${encodeURIComponent(id)}&field=${encodeURIComponent(col)}&value=${encodeURIComponent(v)}`);if(res&&res.status==='ok'){if(isDate&&v){const[y,m,d]=v.split('-');td.textContent=`${d}-${monES[+m-1]}-${y.slice(2)}`;}else{td.textContent=v;}await fetchAndRenderAll(getFilters());}else{td.textContent=old;alert('Error: '+(res?.error||'unknown'));}}catch(e){td.textContent=old;alert('Error de red');}}async function finish(commit){if(isFinished)return;isFinished=true;if(!commit){td.textContent=old;return;}await save();}inp.addEventListener('keydown',e=>{if(e.key==='Enter'){e.preventDefault();shouldSaveOnBlur=false;finish(true);}else if(e.key==='Escape'){e.preventDefault();shouldSaveOnBlur=false;finish(false);}});if(isDate||col==='COMENT.'){inp.addEventListener('change',()=>{shouldSaveOnBlur=false;finish(true);});const handleClickOutside=(e)=>{if(!inp||!inp.parentNode){document.removeEventListener('click',handleClickOutside,true);return;}if(!inp.contains(e.target)&&!td.contains(e.target)){document.removeEventListener('click',handleClickOutside,true);if(shouldSaveOnBlur){const curVal=inp.value||'';const oldVal=isDate?(()=>{const m=/^(\d{2})-(\w{3})-(\d{2})$/.exec(old.trim());if(m){const i=monES.indexOf(m[2].toLowerCase());if(i>=0)return`20${m[3]}-${String(i+1).padStart(2,'0')}-${m[1]}`;}return'';})():old.trim();if(curVal===oldVal){finish(false);}else{finish(true);}}}};setTimeout(()=>{document.addEventListener('click',handleClickOutside,true);},200);}else{inp.addEventListener('blur',()=>{setTimeout(()=>{if(document.activeElement!==inp&&shouldSaveOnBlur)save();},50);});}});
...
if(btnLogin)btnLogin.addEventListener('click',()=>{if(window.google?.accounts?.id){google.accounts.id.initialize({client_id:CLIENT_ID,callback:r=>{idToken=r.credential;if(btnEditMode)btnEditMode.disabled=false;btnLogin.style.display='none';const loginStatus=document.createElement('span');loginStatus.textContent='✓ Sesión iniciada';loginStatus.style.color='#10b981';loginStatus.style.fontWeight='bold';loginStatus.style.marginRight='1rem';btnLogin.parentNode.insertBefore(loginStatus,btnLogin);alert('Sesión iniciada. Activa “Modo edición”.');}});btnLogin.textContent='';btnLogin.style.padding='0';btnLogin.style.border='none';btnLogin.style.background='transparent';google.accounts.id.renderButton(btnLogin,{theme:'outline',size:'large',text:'signin_with',width:200});}else{alert('Falta librería de Google Identity');}});
