...
function widthMap() {
    return {
        ...
        'CANT. ASIG.': 110, 'CANT. SOL.': 110, 'RENGLONES ASIG.': 130, 'RENGLONES SOL.': 130, 'FECHA F8': 110,
        'RECIBO F8': 110, 'ASIGNACIÓN': 110, 'SALIDA': 110, 'DESPACHO': 110, 'FACTURACIÓN': 120, 'EMPACADO': 110,
...
function perRowMetrics(row){
  const complet = !!row['ENTREGA REAL']; const rec = parseIsoDate(row['RECIBO F8']); const end = row['ENTREGA REAL'] ? parseIsoDate(row['ENTREGA REAL']) : new Date();
  const days = (rec && end) ? Math.max(0, Math.round((end - rec) / 86400000)) : '';
  const toNum = v => (typeof v === 'number')?v:parseFloat(String(v||'').replace(',','.'))||0;
  const asig = toNum(row['CANT. ASIG.']), sol = toNum(row['CANT. SOL.']), rasi = toNum(row['RENGLONES ASIG.']), rsol = toNum(row['RENGLONES SOL.']);
  const fillCant = sol > 0 ? Math.round((asig / sol) * 100) : 0; const fillReng = rsol > 0 ? Math.round((rasi / rsol) * 100) : 0;
...
async function renderTable(data, page = 1) {
  ...
  if (!data) {
    if (pgnEl) pgnEl.innerHTML = `<span>Cargando pág ${page}...</span>`;
    const params = new URLSearchParams(getFilters());
    const url = `${A}?route=orders.list&page=${page}&pageSize=${DEFAULT_PAGE_SIZE}&${params.toString()}`;
    const res = await jsonp(url); data = res.data;
  }
  const rawRows = data.rows || []; const W = widthMap();
